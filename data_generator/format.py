# This is for changing txt labels into format for yolo
...
if __name__ == '__main__':
   ...
   for i in range(num_file):
       im = Image.open("~/FishyDetector/data/photo/small_subset/" + file_list[i][:-4] + ".jpg")
       width, height = im.size
       x_scale = 1./width
       y_scale = 1./height
       label_file = open("~/FishyDetector/data/photo/small_subset/" + file_list[i], "a")
       lines = [line.rstrip('\n') for line in open("C~/FishyDetector/data/photo/small_subset/" + file_list[i])]
       num_lines = len(lines)
